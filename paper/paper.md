...
When accessing OME-Zarr data within a ZIP file, in addition to a resource identifier (e.g. file path, URL) pointing to the ZIP file (e.g. ``/path/to/file.zarr.zip``) and the path to the Zarr group “below” the Zarr root (e.g. ``<zarr_root>/path/to/zarr_group/``), the location of the Zarr root within the ZIP file (e.g. ``<zip_root>/path/to/zarr_root/``) needs to be specified. In the absence of a canonical resource identifier (e.g. URI) scheme for OME-Zarr with support for resource identifier chaining (cf. [fsspec](https://filesystem-spec.readthedocs.io/en/latest/), [asdf](https://www.asdf-format.org) URI scheme), **we propose to define the ZIP root as the Zarr root (i.e. with the Zarr root-level ``zarr.json`` file being placed in the ZIP root)**. The addition of a dedicated root directory (e.g. ``myimage.zarr.zip``, with a dedicated ``<zip_root>/myimage.zarr/`` directory containing the Zarr root-level ``zarr.json`` file) may streamline the extraction of data in some cases (cf. "extract here" functionality in some operating systems), but this would come at the cost of the dedicated root directory's name becoming outdated instantly upon renaming the ZIP file. 

Working towards a canonical specification of how to store OME-Zarr data within a single file, we summarized our findings in a comment on the existing “ZipStore” draft for the Zarr specification (https://github.com/zarr-developers/zarr-specs/pull/311#issuecomment-2498071484). Given that our deliberations were not restricted to the OME-Zarr use case, we believe that the Zarr specification would be the right place to make the necessary additions. Going forward, however, we could also envision focusing solely on OME-Zarr, by submitting an RFC.
...
The goal of the OME-Zarr Java ecosystem group was to be able to open an existing Zarr v3, and consequently OME-Zarr v0.5, dataset from [the Fiji image viewer](https://imagej.net/software/fiji/).4

### 2.4.1 Java support for Zarr v3
Discussions started with an update on the current state of work on this topic, which is mainly to be found [here](https://github.com/saalfeldlab/n5-zarr/tree/wip/codecsShards) and [here](https://github.com/zarr-developers/zarr-java). Some existing Zarr v3 implementations (in particular webknossos17) already handle reading and writing of chunks and shards and were identified as a good starting point to prioritize the Java work. Upon common agreement, participants decided to focus hacking on data reading as opposed to data writing.
...
To address the challenges of large-than-memory computing, we applied a simple segmentation task to a real-world bioimaging dataset consisting of a light sheet microscopy image collection. The collection was composed of multiple TIFF files, where each file represented a single 3D volume ((279, 4, 91, 554, 928) dimensions corresponding to time, channel, z, y, and x, for a total size of ~50 GB), together constituting a 4-channel volumetric time-series image (see figure Figure AB). The test data is available on zenodo21. Splitting a large image into a collection of TIFF files in this way is commonly done in bioimaging for easier data handling. Relying on this dataset, our goal was to identify good practices in terms of performance and memory management.
